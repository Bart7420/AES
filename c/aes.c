...
void xor(unsigned char word1[4], unsigned char word2[4], unsigned char result[4]) {
...
unsigned char *keyExpansion(unsigned char *key, int keyLength, int nbRound) {
    ...
    while (i < 4 * (nbRound+1))
    {
        ...
        {   
            rotWord(temp);
            subWord(temp);
            unsigned char rc[4] = {Rcon[i/keyLength], 0x00, 0x00, 0x00};
            xor(temp, rc, temp);
        }
